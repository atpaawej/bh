...
function makeProduct(overrides: Partial<{ id: string; slug: string }> = {}) {
  return {
    ...
    description: 'India\'s first full-stack AI platform with Indic language support.',
...
describe('productService.getBySlug', () => {
  ...
  it('throws not found when the slug does not exist', async () => {
    ...
    await expect(productService.getBySlug('missing-product')).rejects.toMatchObject({
      status: 404,
