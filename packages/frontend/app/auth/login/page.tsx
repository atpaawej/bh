...
function LoginForm() {
  const { loginWithGoogle, loginWithGitHub, sendMagicLink, isAuthenticated, isLoading } = useAuth()
  ...
  async function handleOAuth(provider: 'google' | 'github') {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : 'Unable to start OAuth login')
    }
  }
  ...
  async function handleMagicLink(e: FormEvent) {
    ...
    try {
      ...
      setStatus('error')
      setError(err instanceof ApiClientError ? err.message : 'Unable to send magic link')
    }
  }
  ...
  return (
    <div className="flex min-h-[calc(100vh-8rem)] items-center justify-center px-6 py-16">
      <div className="w-full max-w-md rounded-lg border border-hairline bg-canvas p-8 shadow-sm md:p-10">
        ...
        <h1 className="font-display text-3xl tracking-tight text-ink">Sign in to BharatHunt</h1>
        ...
        {status === 'sent' && (
          <div className="mt-6 rounded-sm border border-deep-green/20 bg-deep-green/5 px-4 py-3 text-sm text-deep-green">
            Magic link sent to <strong>{email}</strong>. Check your inbox and click the link to
            sign in.
          </div>
        )}
