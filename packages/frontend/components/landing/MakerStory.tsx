      ...
      <div className="mx-auto max-w-container px-6">
        <div className="mb-14 max-w-2xl">
          ...
          <p className="mt-5 text-lg text-body-muted">
            Indian makers ship incredible products every week — tools, apps, experiments, and
            quiet revolutions. Most of them never get a proper launchpad. We built BharatHunt so
            they do.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-px overflow-hidden rounded-sm border border-hairline bg-hairline md:grid-cols-3">
          {MOMENTS.map((m) => (
            <article key={m.label} className="bg-canvas p-8 md:p-10">
              <p className="mb-6 font-mono text-xs tracking-[0.12em] text-coral">{m.label}</p>
              <h3 className="mb-3 text-2xl font-medium tracking-tight text-ink">{m.title}</h3>
              <p className="text-sm leading-relaxed text-body-muted">{m.body}</p>
            </article>
          ))}
        </div>
      </div>
